...
// --- MOTOR DE CONEXÃO DERIV CORRIGIDO ---
btnConnectDeriv.addEventListener('click', async () => {
    ...
    try {
        ...
        // CORREÇÃO: URL oficial e limpa utilizando o App ID padrão de testes (1089)
        const appId = '1089';
        const testWs = new WebSocket(`wss://://derivws.com{appId}`);
        
        // Define um tempo limite de 5 segundos. Se a Deriv sumir, ele avisa!
        const timeout = setTimeout(() => {
            testWs.close();
            impedirConexao("O servidor da Deriv demorou muito para responder. Verifique as permissões do seu Token.");
        }, 5000);
        ...
        testWs.onerror = () => {
            ...
            derivStatus.className = "status-offline";
            botLogs.innerHTML += `❌ Falha: Bloqueio de rede ou Token mal formatado.<br>`;
            btnConnectDeriv.disabled = false;
            btnConnectDeriv.innerText = "Conectar Conta";
        };
...
function impedirConexao(mensagem) {
    derivStatus.innerText = "Status: Falha Crítica";
    derivStatus.className = "status-offline";
    botLogs.innerHTML += `❌ Falha: ${mensagem}<br>`;
    btnConnectDeriv.disabled = false;
    btnConnectDeriv.innerText = "Conectar Conta";
}
